...
class Environment:
    ...
    def __init__(self, gridsize=(5, 5), num_mines=5):
    ...
    def observe(self):
        ...
        for i in range(self.num_cases):
            i_pos = (i // self.gridsize[1], i % self.gridsize[1])
            state.append([i_pos, observation[i_pos]])
        return state
    ...
    def act(self, action):
        ...
        if action in self.mines_pos:
            ...
            if action[0]==0 or action[0]==self.gridsize[0] or action[1]==self.gridsize[1] or action[1]==0:
                score+=2
