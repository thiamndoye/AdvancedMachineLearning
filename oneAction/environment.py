...
class Environment:
    ...
    def adjacent(self, position):
        ...
        adjacent_list = []
            ...
            #adjacent_list.append((position[0] + 1, position[1]))
        #if (position[0] + 1, position[1] + 1) in self.positions:
            #adjacent_list.append((position[0] + 1, position[1] + 1))
        #if (position[0] + 1, position[1] - 1) in self.positions:
            #adjacent_list.append((position[0] + 1, position[1] - 1))
        #if (position[0], position[1] - 1) in self.positions:
            #adjacent_list.append((position[0], position[1] - 1))
        #if (position[0], position[1] + 1) in self.positions:
            #adjacent_list.append((position[0], position[1] + 1))
        #if (position[0] - 1, position[1]) in self.positions:
            #adjacent_list.append((position[0] - 1, position[1]))
